...
function onClickBorrar(codigo) {
    ...
    .then(data => {
        if (data.trim() === "ok") {
            ...
            alert("⚠️ No se puede eliminar la materia porque tiene notas registradas.");
        } else if (data.trim() === "tiene_estudiantes") {
            alert("⚠️ No se puede eliminar: la materia tiene estudiantes inscritos.");
        } else {
    ...
    .catch(err => {
        ...
        alert("⚠️ Error en el servidor al eliminar la materia.");
    });
}
